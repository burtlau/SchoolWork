...
def n_gram_precision(reference:Sequence[str], candidate:Sequence[str], n:int) -> float:
    ...
    candidate_group = grouper(candidate, n)
    for each in candidate_group:
        if each in ref_group:
            count += 1
    if len(candidate_group) == 0:
        return 0


    return count / len(candidate_group)
...
def BLEU_score(reference:Sequence[str], candidate:Sequence[str], n) -> float:
    ...
    for i in range(1, n + 1):
        BLEU = BLEU * n_gram_precision(reference, candidate, i)
        print(n_gram_precision(reference, candidate, i), BLEU)
    BLEU = BLEU ** (1 / n)
    BLEU = BP * BLEU
    return BLEU
