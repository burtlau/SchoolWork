...
def parse_arguments(argv: List[str], prog: str = ''):
    """
    ...
    Returns:
        Tuple containing (1) True if all the required arguments are specified
          and False otherwise. (2) A Namespace() object containing the
          user-specified arguments and any optional arguments that take
          default values. (3) A list of any
          unrecognized/unprocessed command-line arguments. (4) An error
          message (if success=False).
    """

    # Initialize the command-line parser.
    parser = argparse.ArgumentParser(
        prog, description='Script for patch match.')
    ...
    parser.add_argument('--w',
                        ...
                        required=False)
    # Rather than generating a new random nearest-neighbor field (NNF) each
    # time PatchMatch is run, you can provide the NNF as an additional input
    # to PatchMatch. This makes it possible to enforce a predictable behavior.
    # The starter code provides initial NNFs for you to use for testing
    # purposes.
    ...
    # Use these flags for debugging purposes, to make sure that your
    # implementations of random search and/or propagation are correct.
...
def main(argv: List[str], prog: str = ''):
    """
    Top-level routine for PatchMatch when running the code from the command
    line.
    ...
    Returns:
        List that holds any unrecognized command-line arguments (in the same
          format as sys.argv[1:])
    """
