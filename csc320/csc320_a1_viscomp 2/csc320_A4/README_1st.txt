...
B. GETTING FAMILIAR WITH THE ALGORITHM'S RESULTS
  ...
  1. I am supplying the full result of running the reference solution on
  my iMac, including all the intermediate results, for the dataset
  test_images/jaguar2. See the algorithm's output after each iteration to
  see what NNF it computes, how it improves after each iteration and how
  the reconstructed source image improves as well.

  2. In addition to the results, I am also supplying the timings of
  running the code on my iMac. These timings should be used as a guide
  only since they may be affected by other programs running on the machine
  concurrently.

C. GETTING FAMILIAR WITH THE STARTER CODE

  1. The code can (almost) be executed out of the box: just complete the
  image reading/writing functions from your A3 implementation into the
  code/patchMatch.py file and the code will be ready to run.

  2. Once you've completed those functions, it is possible to execute the
  code and compute an NNF that is written to output. This NNF will be
  initialized to a random NNF as required by the PatchMatch algorithm but
  will NOT improve after each iteration because the algorithm's main
  iteration does nothing (you need to complete it).

  3. The jaguar2/ dataset requires just a couple of iterations to
  converge. For the other datasets 4-5 iterations will be necessary.
  Experiment with patch sizes on the size of 7 to 15. Use larger patches
  only if necessary and only after you've already debugged your algorithm
...
D. COMPLETING THE STARTER CODE
  ...
  function. This will allow you to assess the quality of the reconstructed
  NNF by looking at how well the source image was reconstructed.

  2. Once you're convinced that function is working correctly, implement
  the random search procedure of patchmatch. After that is done,
  incorporate the propagation procedure as well.
...
2. IMPORTANT: 
  ...
  * The only modification you will do to patchMatch.py
    is to transfer the read/write functionality you implemented 
	in A3 as these are not provided in A4 starting code either.
