...
def get_difference(source, target):
    d = np.abs(source.flatten() - target.flatten())
    d = np.sum(np.where(np.isnan(d), 0, d))
    return d
    ...
    if odd_iteration:
        for i in range(source_row):
            for j in range(source_col):
                ...
                starts_i = np.clip(i + new_f[i, j][0], -source_row, source_row - 1)
                starts_j = np.clip(j + new_f[i, j][1], -source_col, source_col - 1)
                D_0_tar = target_patches[starts_i, starts_j]
...
def random_search(source_row, source_col, source_patches, target_patches, new_f,
    ...
    exp_range = int(np.ceil(-np.log10(w) / np.log10(alpha)))
    if odd_iteration:
        for i in range(source_row):
            for j in range(source_col):
                for exp in range(exp_range):
                    ...
                    wa_iR = w * (alpha ** i) * R
                    u = np.array([i + new_f[i, j, 0] + wa_iR[0].astype(int),
                          j + new_f[i, j, 1] + wa_iR[1].astype(int)])
                    if 0 <= u[0] < source_row and 0 <= u[1] < source_col:
                        D_0_src = source_patches[i, j]
                        x = int(np.clip(i + u[0], -source_row, source_row - 1))
                        y = int(np.clip(j + u[1], -source_col, source_col - 1))
