...
(check-equal? (eva '((λ (y) x) 324)) 'x)
(check-equal? (eva '((λ (y) (λ (x) 324)) 325)) '(λ (x) 324))
(check-equal? (eva '((λ (x) x) ((λ (x) x) 324))) 324)
(check-equal? (eva '((λ (x) x) (λ (x) 324))) '(λ (x) 324))
...
; A function body takes in λ and become a closure
(check-equal? (eva '(((λ (y) (λ (x) y)) 324) 325)) 324)
; A function body takes in another function call and become a closure
(check-equal? (eva '((λ (x) ((λ (y) x) 324)) 325)) 325)
; A function body takes in another unevaluable function
(check-equal? (eva '((λ (y) (λ (z) (λ (w) q))) 324)) '(λ (z) (λ (w) q)))
; A function body takes in nested evaluable body parts
(check-equal? (eva '(((((λ (x) (λ (y) (λ (x) (λ (z) z)))) 321) 322) 323) 324)) 324)
; A function body takes in multiple variables some are same to the pattern
(check-equal? (eva '((λ (x) ((λ (y) (y x y)) 325)) 324)) '(325 324 325))
