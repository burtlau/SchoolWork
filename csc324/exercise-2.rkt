...
(define (tag t)
  (compose (And (lambda(y) ((And list? (Not empty?)) y)) (compose (λ (v) (equal? t v)) first)) ))
(check-false ((tag 'img) 324))
(check-false ((tag 'img) (list)))
(check-false ((tag 'img) (list 'p "hi")))
(check-true ((tag 'img) (list 'img)))
...
(define (Point x y)
  (lambda (msg)
    (match msg
      ...
      ('scale (lambda (k) (Point (* k x) (* k y))))
      ('add (lambda (p) (Point (+ x (p 'x)) (+ y (p 'y))))))))
...
(define (tails e)
(match e
[(list 'and _ ... consequent-expression) (tails consequent-expression)]
[(list 'if _ consequent-expression alternative-expression) (append (tails consequent-expression) (tails alternative-expression))]
[otherwise (list e)]))
