...
(check-equal? (evo `((λ (x) ((λ (y) (set! x (,add1 x))) 2)) 2))
              `(,Void #hash((E1 . ((x #&3) E0))
                      (E2 . ((y #&2) E1))
                      (λ0 . ((λ (x) ((λ (y) (set! x (,add1 x))) 2)) E0))
                      (λ1 . ((λ (y) (set! x (,add1 x))) E1)))))

(check-equal? (evo `((λ (x) ((λ (y) (set! x (,add1 x))) 1)) 1))
              `(,Void
                #hash((E1 . ((x #&2) E0))
                      (E2 . ((y #&1) E1))
                      (λ0 . ((λ (x) ((λ (y) (set! x (,add1 x))) 1)) E0))
                      (λ1 . ((λ (y) (set! x (,add1 x))) E1)))))

; Test literal
(check-equal? (evo 1) `(1 #hash()))

; Test lambda function
(check-equal? (evo '(λ (x) 1)) '(λ0 #hash((λ0 . ((λ (x) 1) E0)))))
(check-equal? (evo '(λ (x) ((λ (x) 1))))
              '(λ0 #hash((λ0 . ((λ (x) ((λ (x) 1))) E0)))))
(check-equal? (evo '(λ (x) ((λ (y) y) 1)))
              '(λ0 #hash((λ0 . ((λ (x) ((λ (y) y) 1)) E0)))))

; Test function
(check-equal? (evo (odd? 2)) `(#f #hash()))
(check-equal? (evo (zero? 10)) `(#f #hash()))


; Test helper function
(define environments-closures #hash((E0 . ((y #&1) E1))
                                    (E1 . ((x #&2) E2))))
...
(check-equal? (get-var 'x (get-env 'E0)) #&2)
(check-equal? (get-var 'x (get-env 'E1)) #&2)
