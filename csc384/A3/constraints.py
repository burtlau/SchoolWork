...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        satAssignments = self.statisfyingAssignments(qi, qj, i, j)
        TableConstraint.__init__(self, name, [qi, qj], satAssignments )

    def statisfyingAssignments(self, qi, qj, i, j):
        satisfyingAssignments = []
        for column_i in qi.domain():
            for column_j in qj.domain():
                if (column_i != column_j and abs(i - j) != abs(column_i - column_j)):
                    satisfyingAssignments.append([column_i, column_j])
...
class NValuesConstraint(Constraint):
    ...
    def hasSupport(self, var, val):
        ...
        varsToAssign = self.scope()
        varsToAssign.remove(var)
        x = findvals(varsToAssign, [(var, val)], satisfyingBounds, satisfyingUpperbounds)
        return x
